    ...
    <div className="border-r-1 border-b-1 border-neutral-800 max-w-100 relative z-1">
      ...
      <section className="p-4 pt-0 text-neutral-400 leading-6 flex flex-col gap-3">
        ...
        <div className="flex flex-col p-1">
          <p>{data.content}</p>
          {(data.members_joined >= data.max_members) && <button className="flex items-center justify-center gap-4 bg-sky-900 text-neutral-50 mt-5 p-2 rounded-3xl">FULLY JOINED <IoGameController/></button>}
          {(data.members_joined < data.max_members) && <button className="flex items-center justify-center gap-4 bg-sky-600 text-neutral-50 mt-5 p-2 rounded-3xl">JOIN FOR FREE <IoGameController/></button>}
